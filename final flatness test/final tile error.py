    ...
    for setting, data in sample_file.items():
        ...
        for sample in data:
            ...
            if tile.is_data_valid:
                ...
                # for side_point_name in tile2._special_side_point_list:
                #     side_point = getattr(tile2,side_point_name)
                #     ax.scatter(side_point.x,side_point.y,side_point.z)
                #     side_distance = list()
                #     side_projected_point =point_to_plane_z_projection(plane,point)
                #     distance = abs(point.z-projected_point.z)
                #     side_distance.append(distance)
                #     side_distance_number = max(side_distance)
                #     side_l2=geometry.Segment3D(side_point, side_projected_point)
                #     ax.plot([side_l2.p1.x,side_l2.p2.x],[side_l2.p1.y,side_l2.p2.y],[side_l2.p1.z,side_l2.p2.z], color='black')
                #     ax.text(side_l2.midpoint.x, side_l2.midpoint.y, side_l2.midpoint.z, "{:.2f}".format(float(distance)), l2.direction_ratio)
                tile_error.plt.show()